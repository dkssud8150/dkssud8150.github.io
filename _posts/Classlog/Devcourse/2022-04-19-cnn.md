...
이 또한, overfitting을 방지하는 방법으로, 층마다 특정 노드를 동작하지 않도록 만든다. dropout은 inference mode에서는 사용하지 않아야 한다.dropout은 fc layer에서만 사용되는데, 그 이유가 conv layer에서는 부분 연결로 인해 부분적으로 잘라서 receptive field가 존재하는데, fc layer의 경우 모든 노드를 다 본다. conv layer에서 dropout을 사용하게 되면 정보의 손실이 발생할 수 있어서 사용하면 안되지만, fc layer에서는 완전 연결, 즉 모든 노드를 다 보기 때문에 몇 개를 동작하지 않게 만들어서 overfitting을 방지하는 것이 좋다.

...
다른 부분을 찾아주고, 어디가 구체적으로 다른지 표시해준다.소스 코드 비교할 때, 설정 파일 비교할 때 많이 사용되며, 실시간으로 비교해준다. `do`를 치면 해당 부분을 바로 카피하고, `dp`를 치면 삭제한다.

## tabpage

분할 할 때마다 원리 창의 크기가 줄어들기에 탭페이지 기능을 많이 사용함
...
:files or :buffers 라고 치면 현재 저장 전인 것들의 리스트를 볼 수 있따.

- % : 현재 편집 중인 버퍼
- # : 바로 이전에 열었던 버퍼, ctrl+6누르면 #이 표시된 파일 열림

파일명 아래 경로명이 적혀있으면 경로명을 인식해서 오픈가능

→ 파일명 위에서 gf(이동) , ctrl+6를 누르면 다시 돌아옴

→ c언어에서 헤더파일을 볼수도 있다. gf

...
- Npos: 전경 점들의 갯수
- binu^(p)_hat, resu^(p)_hat: 전경 점p의 예측된 빈의 수행과 잔류
...
방향을 정제하기 위해, 제안과 GT 박스 사이의 3D IOU가 최소 0.55이상이라는 사실에 기초하여 GT 방향과의 각도 차이`θi^gt - θi`가 [-π/4,π/4] 범위 내에 있다고 가정한다. 그러므로, 빈 크기 *w*와 함께 π/2를 개별 빈으로 나누고, 다음과 같이 빈 기반의 방향 타겟을 예측한다. 
